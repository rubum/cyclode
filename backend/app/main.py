...
app = FastAPI(
    ...
    description="The Event-Driven & Prompt-Driven Autonomous Engineering Platform powered by Antigravity Harness",
    version="1.0.0",
    lifespan=lifespan
)
...
logger = logging.getLogger("cyclode.api")
...
async def global_exception_handler(request: Request, exc: Exception):
    logger.error(f"Unhandled API error on {request.method} {request.url.path}: {exc}", exc_info=True)
    return JSONResponse(
        status_code=500,
